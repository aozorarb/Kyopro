...
void grading(vector<vector<int> >& A,int& correct_count,int& wrong_count/* 呼び出し側に対応するように引数を書く */) {
  ...
  // ここにプログラムを追記
    for (int i=0;i<9;i++) for(int j=0;j<9;j++){
        int ans = (i+1)*(j+1);
        if (ans == A.at(i).at(j))correct_count++;
        else {
            wrong_count++;
            A.at(i).at(j) = ans;
        }
...
int main() {
  ...
  // A, correct_count, wrong_countを参照渡し
    grading(A, correct_count, wrong_count);
